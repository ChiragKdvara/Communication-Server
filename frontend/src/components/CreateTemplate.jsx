...
const CreateTemplate = () => {
  ...
  const validateForm = () => {
    ...
    if (!templateData.template_name || templateData.template_name === false) {
      ...
      validationErrors.template_name =
        "Template name should not exceed 50 characters";
    } else if (/[!@#$%^&*(),.?":{}|<>]/.test(templateData.template_name)) {
      validationErrors.template_name =
        "Template name should not contain special characters except for -, _, and /";
    }
    ...
    if (!templateData.message_title || templateData.message_title === false) {
      ...
      validationErrors.message_title =
        "Message title should not contain special characters except for -, _, and /";
    }

    // Message content validation
    if (
      !templateData.message_content ||
      templateData.message_content === false
    ) {
  ...
  const handleAdd = (variable) => {
    ...
    const newContent = `${message_content.slice(
      0,
      newPosition
    )} {{${variable}}} ${message_content.slice(newPosition)}`;
  ...
  const handleBackspace = (e) => {
    ...
    if (e.key === "Backspace") {
      ...
      if (lastTwoChars === "}}") {
        ...
        const newContent =
          message_content.slice(0, startOfVariable) +
          message_content.slice(caretPosition);
        setTemplateData({ ...templateData, message_content: newContent });
      }
    }
  };
      ...
      <div className="flex w-full">
        <div className="flex justify-between items-start px-4 gap-10 w-full">
          <div className="inputs w-3/4">
            <h2 className="font-semibold uppercase text-secondary mx-1">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col">
              <label htmlFor="temp_name">
                ...
                {errors.template_name && (
                  <span className="text-red-500 font-medium m-0">
                    {" "}
                    ({errors.template_name})
                  </span>
                )}
                {errors.unique_temp_name_error && (
                  <span className="text-red-500 font-medium m-0">
                    {" "}
                    ({errors.unique_temp_name_error})
                  </span>
          ...
          <div className="variables w-1/4">
            ...
            <p className="text-base">
              To add variables click on{" "}
              <span className="font-bold text-secondary">+</span> button
            </p>
              ...
              {variables.map((vars, index) => (
                <div key={index} className="flex flex-col w-3/4 px-2">
                  <div className="flex items-center justify-between h-10">
                    ...
                    <button
                      onClick={() => handleAdd(vars)}
                      className="rounded-full cursor-pointer text-secondary font-bold flex items-center justify-center bg-transparent"
                    >
