...
const Upload = () => {
  ...
  const validateStructure = (jsonData, uploadStep) => {
    if (uploadStep === 1) {
      ...
      const hasHierarchy =
        Array.isArray(jsonData.hierarchy) &&
        jsonData.hierarchy.every((item) => typeof item === "string");
      const hasData =
      ...
      const hasUsers =
        ...
        jsonData.users.every(
          (user) =>
            typeof user.username === "string" &&
            typeof user.email === "string" &&
            typeof user.role === "string" &&
            typeof user.btm_lvl_id === "number"
        );
    ...
    if (selectedFile) {
      ...
      reader.onload = async (event) => {
        try {
          ...
          if (!isValidStructure) {
            setUploadStatus(
              "Error: Invalid file structure. Please check the file and try again."
            );
            return;
          }
          ...
          if (hasNullOrEmpty) {
            setUploadStatus(
              "Error: File contains null or empty values. Please fix and try again."
            );
            return;
          }
          ...
          if (uploadStep === 1) {
            endpoint =
              "http://localhost:8000/api/v1/hierarchy/upload-hierarchy-data";
          } else if (uploadStep === 2) {
    ...
    <div className="h-screen w-full font-poppins">
      ...
      <div className="flex h-[40rem]">
        ...
        <div className="w-1/2 p-4 pl-[4rem] pt-[4rem]">
          ...
          <p>
            Click on the &quote;Upload&quot; button or Drag and Drop the file to
            be uploaded.
          </p>
