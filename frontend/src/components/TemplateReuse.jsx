...
const TemplateReuse = () => {
  ...
  const handleBack = () => {
    if (data?.templates_len == undefined) {
      navigate('/admin', {
        state: {
          templates_len: data?.templates_len,
        },
      })
    } else if (data?.templates_len) {
      navigate('/templates', {
        state: {
          templates_len: data?.templates_len,
        },
  ...
  const handleContinue = () => {
    navigate('/select-filter', {
      state: { ...data, previousPage: `/template/${data.template_id}`, templates_len: data?.templates_len },
    })
  }
          ...
          <div className="flex flex-col h-[410px]">
            ...
            <div className="flex-1 px-4 overflow-y-hidden">
              ...
              <div className="mt-2">
                ...
                <div className="mt-1 border border-gray-300 rounded-md h-[200px] overflow-y-auto p-2">{data.message_content}</div>
              </div>
            </div>
          </div>
        </div>
