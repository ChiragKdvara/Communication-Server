...
const ViewSingleMessage = () => {
    ...
    const fetchReferenceDetails = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL}/api/v1/viewMessages/${id}`
        );
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL}/api/v1/users/user-search`,
          {
  ...
  return (
    <div className="h-screen w-full font-poppins">
      ...
      <div className="bg-primary w-full h-20 flex items-center justify-between">
        <h1 className="m-0 p-4 text-white font-semibold truncate w-1/2">
          {referenceDetails ? referenceDetails.template_name : "Loading..."}
        </h1>
      </div>
              ...
              <div className="flex flex-col w-full gap-2">
                <button className="text-start m-0 border-secondary border-2 text-secondary bg-white font-poppins rounded-[8px] p-2 truncate">
                  ...
                  <span className="font-bold">
                    {referenceDetails ? referenceDetails.btm_lvl : "Loading..."}
                  </span>
                </button>
              </div>
            </div>
            ...
            <div className="flex gap-4 items-center justify-between w-full">
              <div className="flex flex-col gap-2 w-1/3">
                <p className="text-start m-0 bg-primary flex items-center justify-between text-[14px] font-bold text-white font-poppins rounded-[8px] p-2">
                  <span className="font-medium">View Percentage</span>{" "}
                  <span>
                    {referenceDetails
                      ? referenceDetails.read_status_percentage + "%"
                      : "Loading..."}
                  </span>
                </p>
              </div>
              <div className="flex flex-col gap-2 w-1/3">
                <p className="text-start m-0 bg-primary flex items-center justify-between text-[14px] font-bold text-white font-poppins rounded-[8px] p-2">
                  <span className="font-medium">Languages Translated</span>{" "}
      ...
      <div className="mb-2 px-4">
        ...
        >
          <h2 className="text-lg font-semibold">
            Title:{" "}
            {referenceDetails ? referenceDetails.message_title : "Loading..."}
          </h2>
          <svg
        ...
        {isOpen && (
          <div
            className={`flex flex-col items-start justify-between px-4 m-0 border-primary border-solid border-2 rounded-br-[8px] rounded-bl-[8px]
          ${isOpen ? " border-t-0" : ""}
          `}
          >
            <div className="w-3/4 max-h-40 overflow-y-auto">
              {" "}
              <p className="mt-0">
                {referenceDetails
                  ? referenceDetails.message_content
                  : "Loading..."}
              </p>
            </div>
        ...
        <div className="w-1/2 my-1">
          <div className="mb-2">
            ...
            <div className="w-full overflow-y-auto max-h-60">
              {searchPerformed && !searching && filteredUsers.length === 0 && (
                <p className="m-0">No users found</p>
              )}
              {!searching &&
                filteredUsers.map((user) => (
                  <div
                    key={user.exp_message_id}
