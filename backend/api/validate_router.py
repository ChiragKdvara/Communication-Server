...
from sqlalchemy import create_engine, inspect, Table, Column, Integer, String, ForeignKey, MetaData, and_
...
# Assuming that the previously provided code with other endpoints is already imported here
...
async def get_users(
    ...
    reference_id: int = Query(..., description="Reference ID to filter messages")
):
    # Reflect the metadata from the database
    metadata = MetaData(bind=engine)
    metadata.reflect()

    # Fetch the necessary tables and convert user_ids from a string of comma-separated integers to a list of integers
    users_table = Table("users", metadata, autoload_with=engine)
    exp_message_table = Table("exp_message", metadata, autoload_with=engine)
